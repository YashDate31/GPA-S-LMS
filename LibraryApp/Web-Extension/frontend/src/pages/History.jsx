      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex-1">
                        ...
                        <div className="flex flex-wrap gap-2 mb-3 text-xs text-slate-600">
                          <span>Borrowed: {new Date(record.borrow_date).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' })}</span>
                          {record.due_date && <span>• Due: {new Date(record.due_date).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' })}</span>}
                          {record.return_date && <span>• Returned: {new Date(record.return_date).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' })}</span>}
                        </div>
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            {/* Currently Borrowed */}
            <div className="bg-white rounded-xl p-5 border border-slate-100 text-center">
              <p className="text-slate-600 font-medium text-sm mb-2">Currently Borrowed</p>
              <p className="text-4xl font-black text-blue-600">{loanHistory?.currently_borrowed?.length || 0}</p>
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl p-6 border border-slate-100">
            ...
            <div className="flex items-center gap-8">
              {/* Donut Chart */}
              {totalBooks === 0 ? (
                <div className="flex flex-col items-center justify-center bg-slate-50 dark:bg-slate-800 rounded-full w-40 h-40 shrink-0 border border-slate-200 dark:border-slate-700">
                  <BookOpen className="text-slate-300 dark:text-slate-500 mb-2" size={32} />
                ...
                <div className="relative w-40 h-40 shrink-0">
                  ...
                  <div className="absolute inset-0 flex flex-col items-center justify-center">
                    ...
                    <p className="text-[10px] text-slate-500 font-bold uppercase tracking-wider px-2 text-center truncate w-full">{topCategory.name}</p>
                  </div>
                </div>
              )}
              ...
              <div className="space-y-3">
                  {totalBooks > 0 ? (
                    ...
                    <p className="text-sm text-slate-500">Read books to discover your preferences!</p>
                  )}
              </div>
            </div>
          </div>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-3 gap-4">
              ...
              {/* Placeholder badges if none exist and read enough */}
              {badges.length === 0 && (stats.total_books >= 3) && (
                <>
                  <div className="bg-white rounded-xl p-4 border border-slate-100 text-center">
                    <div className="w-12 h-12 rounded-full bg-yellow-50 flex items-center justify-center mx-auto mb-2">
                      <Award className="text-yellow-500" size={24} />
                    </div>
                    <p className="text-sm font-bold text-slate-900">Avid Reader</p>
                  </div>
                </>
              )}
              
              {badges.length === 0 && (!stats.total_books || stats.total_books < 3) && (
                 <div className="col-span-3 text-center py-6 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-100 dark:border-slate-800">
                    <Award className="text-slate-300 dark:text-slate-600 mx-auto mb-2" size={32} />
                    <p className="text-sm text-slate-500 font-medium">Read at least 3 books to start earning badges!</p>
                 </div>
              )}
            </div>
          </div>
