...
class Database:
    ...
    def init_database(self):
        ...
        # Migration: ensure price column exists on books
        try:
            has_price = False
            if self.use_cloud:
                cursor.execute(
                    "SELECT column_name FROM information_schema.columns WHERE table_name='books' AND column_name='price'"
                )
                if cursor.fetchone():
                    has_price = True
            else:
            ...
            if not has_price:
                cursor.execute("ALTER TABLE books ADD COLUMN price REAL DEFAULT 0")
                conn.commit()
                print("Migration: Added 'price' column to books table")
        except Exception as e:
        ...
        try:
            ...
            if self.use_cloud:
                cursor.execute(
                    "SELECT column_name FROM information_schema.columns WHERE table_name='borrow_records' AND column_name='accession_no'"
                )
                if cursor.fetchone():
                    has_accession_no = True
            else:
                cursor.execute("PRAGMA table_info(borrow_records)")
                columns = [col[1] for col in cursor.fetchall()]
                has_accession_no = 'accession_no' in columns

            if not has_accession_no:
                cursor.execute("ALTER TABLE borrow_records ADD COLUMN accession_no TEXT")
                conn.commit()
                print("Migration: Added 'accession_no' column to borrow_records table")
            
            # Always backfill NULL accession_no from book_id (extract first number of range)
            # e.g. "5-14" -> "5", "12" -> "12" for old records
            cursor.execute("""
                UPDATE borrow_records
                SET accession_no = CASE
                    WHEN INSTR(book_id, '-') > 0
                        THEN SUBSTR(book_id, 1, INSTR(book_id, '-') - 1)
                    ELSE book_id
                END
            ...
            print(f"Migration check warning (accession_no): {e}")
    ...
    def _resolve_book_id(self, cursor, book_id_input):
        """Resolve user-supplied book ID (may be a single accession number or a range)
        to the actual book_id stored in the DB. Returns None if not found."""
        bid = str(book_id_input).strip()
        # 1. Exact match on book_id column
        cursor.execute('SELECT book_id FROM books WHERE book_id = ?', (bid,))
        row = cursor.fetchone()
        if row:
            return row[0]
        # 2. Match as an individual accession number inside the barcode CSV field
        #    barcode stores "12792,12793,12794" — use SQLite string trick to avoid partial hits
        cursor.execute(
            "SELECT book_id FROM books WHERE ','||COALESCE(barcode,'')||',' LIKE ?",
            (f'%,{bid},%',)
        )
        row = cursor.fetchone()
        if row:
    ...
    def borrow_book(self, enrollment_no, book_id, borrow_date, due_date):
        ...
        try:
            ...
            cursor.execute(
                "SELECT COUNT(*) FROM borrow_records WHERE enrollment_no = ? AND accession_no = ? AND status = 'borrowed'",
    ...
    def get_book_by_id(self, book_id):
        """Get specific book details by Book ID or individual accession number"""
        conn = self.get_connection()
        cursor = conn.cursor()
        try:
            # Exact match first
            ...
            if row:
                return row
            # Fallback: individual accession number in barcode CSV
    ...
    def get_borrowed_books(self):
        ...
        cursor = conn.cursor()
        cursor.execute('''
            SELECT br.enrollment_no, s.name, s.department, s.year,
                   COALESCE(
